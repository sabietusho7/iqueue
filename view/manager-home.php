...
if (isset($_SESSION["ManagerID"])) {
    ...
    $deskServiceId = $businessController->getBusinessIdByEmployeeId($userId);
    $business = $businessController->getBusinessById($deskServiceId);
    $deskServices = $businessController->getDeskServicesByBusinessId($deskServiceId);
    require_once '../controller/UserController.php';
    $userController = new UserController();
    $manager = $userController->getUserByID($userId);

    if (isset($_GET['modifyBusinessId'])) {
        $deskService = $businessController->getBusinessById($_GET['modifyBusinessId']);
    }
    if (isset($_POST['modify'])) {
        $modify = $_POST['modify'];
        $deskServiceId = $_POST['deskServiceId'];
    }
    if (isset($_POST['registerBusiness'])) {
        header("location:manager-desk-service-info.php");
    }
                                ...
                                <div class="col-lg-12 mb-12">
                                    <div class="card">
                                        <div class="card-header">
                                            <h6 class="text-uppercase mb-0">Desk Services</h6>
                                        </div>
                                        <div class="card-body">
                                            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>"
                                                ...
                                                <table class="table card-text">
                                                    ...
                                                    <?php
                                                    foreach ($deskServices as $deskService) {
                                                        echo "<tr>
                                                 <td>" . $deskService->getId() . "</td>
                                                 <td>" . $deskService->getName() . "</td>
                                                 <td>" . $deskService->getEtc() . " min</td>
                                                 <td>" . $deskService->getSportelist()->getName() . " " . $deskService->getSportelist()->getSurname() . "</td>
                                                 <td style='padding-left:30px;'>";
                                                        if ($deskService->getIsActive()) {
...
//                                                    </label> </td>
                                                            " <td><a href='manager-desk-service-info.php?deskServiceId=" . $deskService->getId() . "'><button type=\"button\" class=\"btn btn-primary\" data-toggle=\"modal\" data-target=\"#\" data-whatever=' " . $deskService->getId() . "' ><i class='fas fa-edit' style='font-size: 20px'></i></button> </a> </td>
